...
interface UseMonthlyExpensesState {
  ...
  updateExpense: (transactionId: string, expenseData: Partial<ExpenseFormData>) => Promise<void>;
...
export function useMonthlyExpenses(initialMonth?: string): UseMonthlyExpensesState {
  // Estado principal
  ...
  const [selectedMonth, setSelectedMonthState] = useState(initialMonth || '2025-07');
  ...
  const loadExpenseData = useCallback(async (month: string) => {
    try {
      ...
      console.log(`✅ Gastos cargados: ${monthlyData.transactions.length} transacciones`);
  ...
  const addExpense = useCallback(async (expenseData: ExpenseFormData) => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Error agregando gasto';
   ...
   */
  const initializeMonth = useCallback(async (month: string) => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error inicializando mes');
    }
  }, [loadExpenseData]);
   ...
   */
  const getTotalByCategory = useCallback((categoryName: string): number => {
    if (!expenseData) return 0;
    
    const categoryData = expenseData.summary.find(s => s.category_name === categoryName);
   ...
   */
  const getTransactionsByCategory = useCallback((categoryName: string): ExpenseTransaction[] => {
    if (!expenseData) return [];
    
    return expenseData.transactions.filter(t => t.category_name === categoryName);
  }, [expenseData]);
