...
import { getAvailableMonths } from '@/lib/services/budget';
...
export interface DashboardData {
  ...
  budgetData: any; // MonthlyBudgetData | null
  expenseData: any; // MonthlyExpenseData | null
  incomeData: any; // Ingreso[]
  debtData: any; // Deuda[]
  isLoading: boolean;
  error: string | null;
 ...
 */
export const useDashboardData = (initialMonth: string = '2025-07'): DashboardData => {
  ...
  const isLoading = budgetHook.isLoading || expenseHook.loading || incomeHook.loading;
  ...
  const calculateSummary = useCallback((): DashboardSummary => {
    ...
    // Datos de ingresos
    const totalIncome = incomeHook.ingresos?.reduce((sum, ingreso) => sum + ingreso.monto, 0) || 0;
    
    // Datos de deudas
    const totalDebt = incomeHook.deudas?.reduce((sum, deuda) => sum + deuda.monto, 0) || 0;
    ...
    const overBudgetCount = budgetHook.categories?.filter(category => {
      // Obtener gastos reales para esta categoría
      const categoryExpenses = expenseHook.expenseData?.summary?.find(
  ...
  }, [budgetHook.budgetData, budgetHook.categories, expenseHook.expenseData, incomeHook.ingresos, incomeHook.deudas]);
