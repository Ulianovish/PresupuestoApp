 ...
 */
async function migrateExpense(userId: string, expense: ExpenseFormData): Promise<string | null> {
  try {
    console.log(`💰 Migrando: ${expense.description} - ${expense.amount.toLocaleString('es-CO')}`);
    ...
    console.error(`❌ Error inesperado migrando ${expense.description}:`, error);
    return null;
  }
}
...
export async function migrateJulyExpenses(): Promise<{
  ...
  try {
    ...
    if (hasExistingData) {
      const message = 'Ya existen gastos para julio 2025. La migración se canceló para evitar duplicados.';
    ...
    if (success) {
      ...
      console.log('Los gastos de julio 2025 están ahora disponibles en Supabase.');
...
export async function checkMigrationStatus(): Promise<{
  ...
  try {
    ...
    const totalAmount = data?.reduce((sum: number, expense: any) => sum + expense.amount, 0) || 0;
...
export async function deleteJulyExpenses(): Promise<boolean> {
  try {
    ...
    if (gastos && gastos.length > 0) {
      const gastoIds = gastos.map((g: any) => g.id);
...
if (typeof window !== 'undefined') {
  ...
  (window as any).migrateJulyExpenses = migrateJulyExpenses;
  (window as any).checkMigrationStatus = checkMigrationStatus;
  (window as any).deleteJulyExpenses = deleteJulyExpenses;
